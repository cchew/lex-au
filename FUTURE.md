...
**Planned fix, targeted for v0.10.0 (B4), not yet shipped as of v0.9.3:** `_build_schedule_content` will run quoted-structure preprocessing over schedule groups, wrap amendment-instruction items as their own container instead of the literal instruction number, and skip schedule-TOC-styled paragraphs — collapsing the ~98.7% false-positive share. lex-au-explorer's `build/bundle.py` `_schedule_units`/`build_toc` will need updating to walk the resulting nesting; its `~N` eId-disambiguation workaround will become largely redundant (harmless to leave in) once B4 lands. See the P1 note's "Fix-set recommendation" (B4) for the full mechanism table; this entry will be updated to "resolved" once B4 ships and the dataset CHANGELOG note ("schedule clause eIds renumbered in v0.10.0; see B4") is applied on export.
