...
## Converter flattens schedule Part/Division numbering, producing duplicate `clause-N` eIds

Found 2026-09-05 while implementing eId-collision disambiguation in lex-au-explorer (`docs/superpowers/sdd/2026-09-04-schedule-and-figure-rendering/task-4-report.md` in the EA project wrapper). A schedule's `<hcontainer name="clause">` children are flattened into one flat list of direct siblings under the schedule, with the source AKN's `<part>`/`<division>` grouping dropped from the corpus XML. When a clause number legitimately repeats across two different Parts/Divisions of the same schedule (a common drafting pattern — "clause 30" of Part 2 and "clause 30" of Part 3, say), the flattened `eId` (`schedule-N__clause-<num>`) collides, because nothing in the flattening preserves which Part/Division a clause number was scoped to.

Confirmed example: `a-new-tax-system-(family-assistance)-act-1999.xml`, `schedule-1` (107 direct clauses) has exactly one such collision — `eId="schedule-1__clause-30"` occurs twice, at document positions 63 ("Standard rate", a short rate table) and 96 (a large, unrelated definitions clause). Both are real, distinct clauses with entirely different content sharing one eId.

Not fixed here — this is a corpus/converter defect, not a consumer bug; lex-au-explorer's `build/bundle.py` works around it downstream by disambiguating the second occurrence's bundle key (`~2` suffix) rather than losing it, but a proper fix belongs in whatever produces the flattened `<clause>` eId scheme in the first place (retain the source Part/Division context, or number `clause-N` uniquely per schedule at ingest time). Scan for other schedules with the same defect not yet done — this was found via one targeted `Counter` check on one Act, not a corpus-wide sweep.
