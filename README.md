...
- **v0.9.0** - Cross-reference injection no longer relocates references to the end of a mixed-content paragraph (`reflinks._process_p` now splices at document position). Schedule tables are now rendered in the AKN output, recovering roughly 125k table paragraphs across the tariff, appropriation, supply, repeal and family-law superannuation Acts. DOCX-to-AKN fidelity audit tooling added (`scripts/audit_conversion_fidelity.py`, `src/lexau/fidelity.py`); corpus-wide summary in `corpus/reports/fidelity/SUMMARY.md`. Full corpus re-ingested: `reorder` paragraphs fell from ~20.1k to ~0.9k and `drop_para` from ~343k to ~51k against the fixed corpus.
